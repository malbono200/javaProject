...
public class MergeSort { //정렬을 위한 공간이 하나 더 필요한 정렬
	...
	private static void mergeSort(int[] a, int left, int right) {
		if (left < right) {
			int[] currentTarget = Arrays.copyOfRange(a, left, right + 1);
			System.out.println("분할 대상: " + Arrays.toString(currentTarget) + " (인덱스 " + left + "~" + right + ")");
		}
	...
	private static void merge(int[] a, int left, int mid, int right) {
		int[] leftPart = Arrays.copyOfRange(a, left, mid + 1);
		int[] rightPart = Arrays.copyOfRange(a, mid + 1, right + 1);
		System.out.println("병합 진행: 왼쪽 " + Arrays.toString(leftPart) + " + 오른쪽 " + Arrays.toString(rightPart));
		...
		int[] mergedPart = Arrays.copyOfRange(a, left, right + 1);
		System.out.println("병합 완료: " + Arrays.toString(mergedPart));
		System.out.println();
	}
}
